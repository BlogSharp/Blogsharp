namespace BlogSharp.Core.Impl.Tests.Services.FileSystem
{
    ...
    public class TransactionalFileServiceTests
    {
        ...
        public void SetUp()
        {
            ...
            System.IO.File.Delete(this.textfile);
            using (StreamWriter sw = System.IO.File.CreateText(this.textfile))
            {
                sw.Write("blah");
            }
        }
        ...
        public void Search_recursive_should_search_recursive()
        {
            ...
            using (TransactionScope tran = new TransactionScope())
            {
                var files = this.fileService.SearchDirectory(dirInfo.FullName, "*.*", SearchOptions.File, SearchLocation.Recursive);
                Assert.That(files.Count(), Is.EqualTo(2));
                this.EnumerablePathEquals(files, dirInfo.FullName, new[] { "sub1" + seperator + "b.txt", "sub2" + seperator + "a.txt", });
