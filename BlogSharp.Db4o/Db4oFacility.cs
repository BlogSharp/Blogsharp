namespace BlogSharp.Db4o
{
	...
	public class Db4oFacility : AbstractFacility
	{
		...
		{
			this.Kernel.Register(Component.For<IObjectContainerWrapper>()
		...
		protected virtual void ConfigureFacility()
		{
			...
			this.Kernel.Register(Component.For<IObjectContainerManager>().ImplementedBy<DefaultSessionManager>());
			this.Kernel.Register(
				Component.For<IObjectContainerProviderProvider>().ImplementedBy<DefaultContainerProviderProvider>());
			string store = this.FacilityConfig.Attributes["store"];
			this.Kernel.Register(Component.For<IObjectContainerStore>().ImplementedBy(Type.GetType(store)));
			foreach (IConfiguration factoryConfig in this.FacilityConfig.Children)
			{
				...
				this.ConfigureObjectServers(factoryConfig, firstFactory);
				firstFactory = false;
			}
		}
		...
		{
			var providerProvider = this.Kernel.Resolve<IObjectContainerProviderProvider>();
			...
			this.Kernel.AddComponentInstance(String.Format("{0}.cfg", id), cfg);
				...
				case Db4oMode.RemoteServer:
					{
						...
						this.Kernel.Register(Component.For<IObjectContainerProvider>()
						                     	.Named(providerName)
				...
				case Db4oMode.EmbeddedServer:
					{
						...
						this.disposeAction += () => objectServer.Ext().Close();
						this.Kernel.Register(Component.For<IExtObjectServer>()
			...
			providerProvider.AddProvider(alias, this.Kernel.Resolve<IObjectContainerProvider>(providerName));
		}
