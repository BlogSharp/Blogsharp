...
namespace BlogSharp.Core.Impl.Services.FileSystem
{
	public class TransactionalFileService : IFileService
	{
		...
		protected virtual Stream OpenFileUnManaged(string source, FileMode fileMode, FileAccess fileAccess, FileShare fileShare)
		{
		...
		protected virtual SafeFileHandle GetFileHandle(string source, KtmTransactionHandle tranHandle, FileMode fileMode, FileAccess fileAccess, FileShare fileShare)
		{
		...
		protected virtual SafeFileHandle GetFileHandleForInfo(string source, KtmTransactionHandle tranHandle, out NativeMethods.WIN32_FIND_DATA data)
		{
			var safeHandle = NativeMethods.FindFirstFileTransacted(source, NativeMethods.FINDEX_INFO_LEVELS.FindExInfoStandard,
												  out data, NativeMethods.FINDEX_SEARCH_OPS.FindExSearchNameMatch,
		...
		#region IFileService.CreateFile Members
		...
		public IEnumerable<IFileSystemInfo> SearchDirectory(string directory, string searchPattern,SearchOptions searchOptions,SearchLocation searchLocation)
		{
			using (var tranHandle = GetKtmTransactionHandle())
			{
				...
				while(directoriesToBeSearched.Count>0)
				{
					...
					using (var directoryHandle = GetFileHandleForInfo(directory + Path.DirectorySeparatorChar + @"*", tranHandle, out win32findData))
					{
